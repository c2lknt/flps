...
import LanguageMenu from './components/languages';
import nlogo from './images/Nblack.png'
import FlpsCard from './components/card'
...
function App(props) {
  ...
  const codesWithCounts = Object.keys(codesFlat).reduce(function(obj, c) {
    // console.log(wholeFile.codes[0][c])
    const retVal = wholeFile.codes[0][c] ? wholeFile.codes[0][c].length : '0'
    obj[c] = retVal
    return obj
  }, {});
  const drawer = (
    <div>
      ...
      <Codes setCode={setCode} codes={pyCodes} counts={codesWithCounts} />
    </div>
  );
  ...
  return (
    <div className={classes.root}>
      ...
      <main className={classes.content}>
        ...
        <Paper elevation={3} variant="outlined" className={classes.langheader}><Typography variant="h4">{lang + (code !=='' ? ', ' + codesFlat[code] : '') + (date !== [0,2000] ? ', ' + date[0] + ' - ' + date[1] : '')} </Typography></Paper>
